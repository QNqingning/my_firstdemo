...
class EquipmentApplyList extends Lock{
	public function index(){
		
		$list = db("equipment_apply")->select();
		$total = db("equipment_apply")->count();
	...
	public function edit(){
		...
		if(db("equipment_apply")->where('id',$get['id'])->setField('check_result',$get['result'])){
	...
	function SendMail($data=[]){
        ...
        if($data && is_array($data)){
            foreach ($data as $k=>$v){
                ...
                $mail->Body = '您的设备申请'. $v['content']; //邮件主体内容

                //发送成功就删除
                if ($mail->Send()) {
                   	$this->redirect("EquipmentApplyList/index");
                }
            }
        }
    }
